...
enum ExtensionPackageDownloader {
    ...
    private static func latestGitHubReleaseTag(repository: String) async throws -> String {
        let release = try await fetchLatestGitHubRelease(repository: repository, as: GitHubReleaseTag.self)
        let tag = release.tagName
        return tag.hasPrefix("v") ? String(tag.dropFirst()) : tag
    }

    private static func latestGitHubAssetURL(repository: String, assetNameContains: String) async throws -> URL {
        let release = try await fetchLatestGitHubRelease(repository: repository, as: GitHubRelease.self)
    ...
    private static func fetchLatestGitHubRelease<T: Decodable>(
        repository: String,
        as type: T.Type
    ) async throws -> T {
